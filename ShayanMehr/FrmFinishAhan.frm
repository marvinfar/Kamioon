...
'
 ...
 '''
       ...
       '
        If rs(0) < 0 Then
         .Cell(.Rows - 1, 3).Text = "«÷«›Â Ê“‰"
         .Cell(.Rows - 1, 2).Text = Abs(rs(0))
       ElseIf rs(0) > 0 Then
         .Cell(.Rows - 1, 3).Text = "ò”— Ê“‰"
         .Cell(.Rows - 1, 2).Text = rs(0)
       Else
         .Cell(.Rows - 1, 3).Text = "Å«Ì«Å«Ì"
         .Cell(.Rows - 1, 2).Text = rs(0)
       End If
  ''''''''''''''''''''''''''''''''''''''''''''''''''''''''
       ''''
